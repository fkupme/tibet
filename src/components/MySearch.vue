<template>
  <section>
    <form class="search" :class='{"search_active":show && isMobile}'>
      ...
      <my-select type='dropdown' :class='{"select-label_active":show && isMobile}' v-model='location' :options='locationOptions' :id='locationID'>выберите из списка</my-select>

      <my-select :class='{"select-label_active":show && isMobile}' v-model='date' :id='dateID' type="date">укажите диапазон</my-select>

      <my-select :class='{"select-label_active":show && isMobile}' v-model='team' :options='teamOptions' :id='teamID' type="dropdown">минимум 4 человека</my-select>
